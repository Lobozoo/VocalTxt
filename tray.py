...
def _find_ico() -> str | None:
    """Locate talkloom.ico: next to the script, or inside PyInstaller bundle."""
    import sys, os
    for base in (getattr(sys, "_MEIPASS", ""), os.path.dirname(os.path.abspath(__file__))):
        p = os.path.join(base, "talkloom.ico")
        if os.path.exists(p):
            return p
    return None
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, app_ctrl, parent=None):
        ...
        self.setWindowTitle("Talkloom Settings")
        ...
        self.startup_chk = QCheckBox("Start Talkloom with Windows")
        ...
        quit_btn = QPushButton("Quit Talkloom")
        ...
        about = QLabel(
            "Talkloom — local, offline dictation. Hold the hotkey, speak, "
    ...
    def _info_html(self) -> str:
        ...
        html = f"""
        <p {H}>&#127908;&nbsp; Talkloom</p>
        <p {P}><span {HL}>Talkloom is a voice-to-text dictation tool that
        ...
        <span {HL}>%APPDATA%\\Talkloom</span>.</p>
        {RULE}
        ...
        <span {KEY}>{addw}</span>. Talkloom then steers transcription toward
    ...
    def _reset_all_data(self):
        ...
        QMessageBox.information(self, "Talkloom", "All data has been reset.")
...
class Tray(QSystemTrayIcon):
    ...
    def __init__(self, app_ctrl, parent=None):
        ...
        self.app_ctrl = app_ctrl
        self.setToolTip("Talkloom — hold hotkey to dictate, click for settings")
