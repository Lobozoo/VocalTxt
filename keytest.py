...
both raw and canonicalised, plus the normalised name EchoScribe's hotkey.py
would use. Run it, hold Ctrl+Alt+Space, release, then press Esc to quit.
"""

from pynput import keyboard
from hotkey import _key_name  # reuse the exact same normalisation EchoScribe uses
...
def on_press(key):
    ...
    if {"ctrl", "alt", "space"} <= pressed:
        print(">>> COMBO DETECTED — EchoScribe would start recording here <<<")
